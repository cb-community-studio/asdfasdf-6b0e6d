...
import asdadsfFakerFactory from "./asdadsfFakerFactory";
...
const AsdadsfFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = asdadsfFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("asdadsf").create(elem));
...
export default connect(null, mapDispatch)(AsdadsfFakerDialogComponent);
