...
import seederData from "./asdadsf.seeder.json";
...
const AsdadsfSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("asdadsf").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(AsdadsfSeederDialogComponent);
